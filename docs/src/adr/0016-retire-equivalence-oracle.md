...
The distinction is the whole of the decision. The oracle produced two things: a *live* differential (the Rust engines compared against a running Python reference over a generated input domain) and a set of *frozen goldens* (byte-pinned observable output: the event-stream fingerprints, the database-state snapshots, the HTTP-response fingerprints, the event-schema contract snapshot, the normaliser conformance table, and the recorded-corpus replay goldens). (The OpenAPI contract snapshot was also frozen at this point; it was later retired along with the in-process HTTP transport, whose last internal caller it described, when the frontend moved to typed commands, see [ADR-0019](0019-typed-command-facade.md).) Only the live differential needed the interpreter. The frozen goldens are bytes, and the Rust side already reimplements the normaliser and every emitter that produces them, so a hermetic Rust test reproduces each golden with no second implementation present. Retirement keeps every frozen golden and the hermetic tests that assert them, and drops only the live differential and the Python that backed it.

Concretely:

- The committed goldens move out of the retired tree into the Rust workspace: the replay corpus under `frontend/src-tauri/fixtures/corpus/` (per-scenario event-stream fingerprints, database-state snapshots, and per-endpoint HTTP-response fingerprints), the ratified contract snapshot under `frontend/src-tauri/contracts/`, and the normaliser and listener-family fixtures under `frontend/src-tauri/eo-wire/tests/fixtures/`. The hermetic tests that assert them (`corpus_replay_oracle.rs`, `emitters_proof.rs`, `conformance.rs`, `yml_family.rs`, `event_schema_conformance.rs`) are repointed and continue to pass byte-for-byte with no Python.
...
- `frontend/src-tauri/fixtures/corpus/` (the frozen event-stream, database-state, and HTTP-response goldens)
