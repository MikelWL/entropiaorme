...
- In-process dispatch is sub-millisecond per request at p50 and p95 across the hydration surface, with no socket or serialisation hop. (This was measured on the in-process HTTP router, since retired with the `eo-http` crate under [ADR-0019](0019-typed-command-facade.md); the typed commands that replaced it dispatch as direct in-process calls.)
