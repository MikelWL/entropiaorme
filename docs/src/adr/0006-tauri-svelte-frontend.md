...
> **Transport update ([ADR-0013](0013-in-process-collapse.md)).** The frontend decision below is unchanged, and its central bet was vindicated: the contract was stable enough that the backend changed underneath it without the frontend noticing. The context's "the backend is being migrated ... proxying the remainder" is now history; the migration is complete, the sidecar and proxy are gone, and the frontend reaches the now-native backend in-process rather than over HTTP. Read the present-tense migration prose below as the situation at the time of the decision.
>
> **Client update ([ADR-0019](0019-typed-command-facade.md)), superseding the generated-client parts below.** The "generated, not hand-written" HTTP client described under Decision and Consequences has itself been superseded. The frontend no longer builds HTTP-shaped requests against an OpenAPI snapshot: it invokes typed Tauri commands whose TypeScript bindings are generated from the `eo-api` command manifest and Rust DTOs by `cargo xtask gen-ts`, and the `openapi-typescript`/`openapi-fetch`/`schema.d.ts`/`gen:api:check` toolchain named below has been removed. The compile-time-checked-contract property the decision sought is preserved and strengthened: a field-name or type mismatch is now a compile error on both sides of the language boundary. Read the OpenAPI-codegen prose below as the mechanism at the time of the decision.
