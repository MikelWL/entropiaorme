...
- The database carries a statistics and maintenance posture: analytical indexes are added forward-only through the migration chain, `ANALYZE` runs once as part of the migration that introduces them (so the planner has selectivity statistics from first use), and `PRAGMA optimize` runs on a clean shutdown to keep those statistics current over the database's life. The maintenance calls stay off the hot path: statistics are gathered at migration time and refreshed once at exit, never per request.
