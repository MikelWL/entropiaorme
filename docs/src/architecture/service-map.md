...
- **Planet maps and traversal.** `planet_maps` loads the bundled raster catalogue and calibrated coordinate bounds; `map_pins` owns durable pin storage, spatial reads, duplicate-neighbour queries, and independent named pin sets over each planet, and projects each pin's latest visit, cooldown, and its configuration's colour/category/special kind. `pin_configs` owns the per-preset pin palette (the pin *types*; placed pins reference one, and its colour and behaviour flow through to them) with an explicit cascade to placed pins on delete. `navigation` owns the persisted route lifecycle, deterministic open-route optimisation over special-tree pins, fifteen-unit arrival matching (distinct from the five-unit duplicate-neighbour advisory), harvesting-driven progress, manual coordinate refresh, and radar calibration. The facade owns calibrated-bound validation and the generic/special configuration invariants, and exposes typed pin-configuration, route, and calibration commands.
