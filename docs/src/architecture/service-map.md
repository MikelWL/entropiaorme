...
- **Planet maps and traversal.** `planet_maps` loads the bundled raster catalogue and calibrated coordinate bounds; `map_pins` owns durable pin storage, spatial reads, duplicate-neighbour queries, and independent named pin sets over each planet. `navigation` owns the persisted route lifecycle, deterministic open-route optimisation, two-unit arrival matching, harvesting-driven progress, manual coordinate refresh, and radar calibration. The facade owns calibrated-bound validation and exposes typed route and calibration commands.
