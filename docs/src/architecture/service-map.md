...
- **Window and overlay chrome.** The Tauri command handlers `toggle_overlay`, `toggle_cartography_overlay`, `show_scan_overlay`, and `hide_scan_overlay` manage the pre-spawned hidden overlay windows; on Windows a runtime-icon installer sets DPI-appropriate window icons. The cartography overlay is separately permission-scoped and reuses the typed coordinate-scan and pin-create operations rather than owning backend logic.
...
- **Planet maps.** `planet_maps` loads the bundled raster catalogue and calibrated coordinate bounds; `map_pins` owns durable pin storage and independent named pin sets over each planet, while the facade owns calibrated-bound validation.
...
| Maps | planet-map catalogue and coordinate scan; pin list/create/update/delete; named map list/create/rename/delete; calibration start/cancel/status |
