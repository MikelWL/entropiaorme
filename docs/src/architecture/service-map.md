...
- **The IPC dispatch and the event bridge.** The backend surface is typed commands (`app/src-tauri/entropia-orme/src/commands.rs`), one thin `#[tauri::command]` per operation delegating to the composed `eo_api::Api` against the published substrate state. Raw capture and map-image bytes cannot ride a JSON DTO, so `capture_png` and `planet_map_image` remain bespoke byte commands; shell-owned window toggles are likewise outside the domain manifest. The build-time application-command catalogue generates the Tauri ACL, and a parity test holds that catalogue to the registered typed and bespoke surface. Trusted windows receive the full command group; the cartography webview receives only its read and drop surface: `planet_maps_list`, `map_views_list`, `pin_configs_list`, `maps_scan_coordinates`, `map_pin_nearby`, and `map_pin_create`. The companion `spawn_domain_event_bridge` subscribes to the event-stream hub and re-emits each frame onto the Tauri event system, the in-process replacement for the frontend's former HTTP event stream.
...
- **Planet maps and traversal.** `planet_maps` loads the bundled raster catalogue and calibrated coordinate bounds; `map_pins` owns durable pin storage, spatial reads, duplicate-neighbour queries, and independent named pin sets over each planet, and projects each pin's latest visit, cooldown, and its configuration's colour/category/special kind. `pin_configs` owns the per-preset pin palette (the pin *types*; placed pins reference one, and its colour and behaviour flow through to them) with an explicit cascade to placed pins on delete. `navigation` owns the persisted route lifecycle, deterministic open-route optimisation over special-tree pins, five-unit arrival matching, harvesting-driven progress, manual coordinate refresh, and radar calibration. The facade owns calibrated-bound validation and the generic/special configuration invariants, and exposes typed pin-configuration, route, and calibration commands.
...
| Maps | planet-map catalogue and coordinate scan; pin list/viewport/nearby/create/update/delete; named map list/create/rename/delete; coordinate calibration start/cancel/status; navigation snapshot/start/update/skip/undo/pause/replan/end; radar calibration start/cancel/status/geometry |
