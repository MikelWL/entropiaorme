...
- **Planet maps.** `planet_maps` loads the bundled raster catalogue and calibrated coordinate bounds; `map_pins` owns durable pin storage, independent named pin sets over each planet, and map-space validation.
