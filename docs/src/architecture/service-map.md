...
- **Market data.** `market_paste` (the parser over the game's market-ledger clipboard export) and `market_service` (the paste feed and its overview, history, break-even, and mob-ranking reads). An informational layer: estimated markup never joins the ledger or the analytics aggregates, a boundary the `market-isolation` CI guard enforces. The full pipeline, including the optional shared service, is the [market data chapter](market-data.md).
...
| Analytics | period-scoped `overview`; `hunting`; period-scoped `harvest`; harvest stock read/update; the ledger (paged list, whole-ledger `summary`, create, entry delete), presets (list, create, delete), and inventory (list, create, item patch/delete, item `sell`) |
