...
The shell pre-spawns four application webviews: the main application, the tracking strip, the manual-scan overlay, and the cartography pin overlay. The three overlays start hidden, undecorated, transparent, always on top, and absent from the taskbar. The cartography overlay is a deliberately small action surface: its shared preference holds the selected planet and a sanitised palette of pin definitions, while each click uses the same coordinate-capture command and typed pin-create command as the Maps page. Its frontend-only preference and pin-change events keep the Maps page and overlay in step without widening the backend domain-event taxonomy. A dedicated Tauri capability grants that webview only its own hide, resize, drag, event, path, and preference-store operations. The application-command ACL separately limits it to the map catalogue read, coordinate scan, and pin creation; it does not inherit the trusted windows' remaining application commands, shell-open access, or child-window permissions.
...
- The producer spine and the facade share live service handles. The same `Arc<HuntTracker>`, settings writer, skill tracker, and hotbar listener are cloned into the facade, so the commands and the producer-side bus subscriptions operate on one instance each.
