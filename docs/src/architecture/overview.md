...
The frontend reaches the backend through one seam: typed Tauri IPC commands (`frontend/src-tauri/entropia-orme/src/commands.rs`), one per operation, each delegating to the composed service facade (`eo-api::Api`). The webview invokes a named typed command with typed arguments and receives a typed value back; a field-name typo is a compile error rather than a runtime `undefined`. The sole non-typed command is `capture_png`, which returns raw image bytes. Change notifications travel the other way over the Tauri event system. From the frontend's point of view there is one backend it calls and one event bus it listens on.

## The in-process dispatch facade

`eo-api::Api` (`frontend/src-tauri/eo-api/src/lib.rs`) is the application boundary the typed commands dispatch into: each command is a thin `#[tauri::command]` that calls one async facade method, which orchestrates the domain services (`eo-services`) and returns a typed DTO or the typed `ApiError` contract. There is no socket, no router, and no HTTP envelope: a request is a direct in-process call, and serialisation happens exactly once, at the command boundary. The DTOs are plain `serde` structs whose TypeScript bindings are generated from the Rust source by `cargo xtask gen-ts`, held in lock-step with the Rust manifest by a CI check.
