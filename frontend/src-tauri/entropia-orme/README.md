...
- **`build-dev-config.mjs`**: generates `tauri.dev.local.json` ahead of each dev launch. Writes the env-driven `build.devUrl` (`http://localhost:<port>` driven by `ENTROPIAORME_FRONTEND_PORT`, matching the port Vite binds). Tauri 2 cannot interpolate env vars inside config field values, so this generated overlay is the shim that keeps the dev URL env-driven.
...
- **Changing the dev port**: set `ENTROPIAORME_FRONTEND_PORT` in the environment (`.env.local`). Do not edit `tauri.dev.local.json` directly; it is regenerated on every dev launch.
