//! Native tracking session-read computation:
//! the session-list and session-detail projections
//! ([`list_sessions_impl`], [`get_session_impl`]).
//!
//! The live tracking HTTP routes have migrated onto the typed-command facade
//! (`eo_api::tracking`); these two projections are retained because the
//! guide-mode demo read namespace (`crate::demo`) serves them over its parallel
//! database, keeping the demo and the facade byte-for-byte aligned.
//!
//! Both reads are SQL aggregation over the shared database (and, for the
//! session detail, the skill-calibration table) plus an injected `now` for an
//! active session's running duration, shaping the result to the
//! `TrackingSession` / `SessionDetail` response models byte-for-byte.
//!
//! The fidelity cruxes:
