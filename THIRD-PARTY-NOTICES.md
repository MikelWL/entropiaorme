...
- **File:** `app/src-tauri/entropia-orme/resources/models/svtrv2_rec.onnx` (~24 MB)
...
- **Used by:** the native OCR engine (`app/src-tauri/eo-services/src/ocr_engine.rs`): skill / profession panel scans and repair-window cost reads.
...
- **File:** `app/src-tauri/entropia-orme/resources/models/ppocr_keys_v1.txt`
...
- **Used by:** the native OCR engine (`app/src-tauri/eo-services/src/ocr_engine.rs`) as the recognition model's decode alphabet.
- **License:** Apache License 2.0
...
The application is a single Rust binary (the Tauri shell and the `app/src-tauri/` cargo workspace) with a Svelte frontend. Its Rust dependencies (crates.io) and npm dependencies (`app/package.json`) are pulled under their published licenses; the Tauri framework itself is dual-licensed Apache 2.0 / MIT.
