...
class TestQuestCRUD:
    ...
    def test_update_quest(self, quest_service: QuestService):
        ...
        updated = quest_service.update_quest(q["id"], {"name": "Updated", "planet": "Arkadia"})
        assert updated["name"] == "Updated"
        assert updated["planet"] == "Arkadia"

    def test_update_quest_skill_reward_clears_expected_markup(self, quest_service: QuestService):
        q = quest_service.create_quest({
            "name": "Original",
            "reward_ped": 5.0,
    ...
    def test_row_to_quest_normalizes_numeric_string_id(self, quest_service: QuestService):
    ...
    def test_scalar_from_row_returns_none_for_malformed_empty_tuple(self, quest_service: QuestService):
...
class TestQuestActions:
    ...
    def test_complete_skill_quest_creates_quest_claim_not_ledger(self, quest_service: QuestService):
...
class TestQuestSchemaMigration:
    def test_cancel_completed_quest_without_reward_undo_keeps_ledger(self, quest_service: QuestService):
        q = quest_service.create_quest({
            "name": "Reward Quest",
            "cooldown_hours": 24,
            "reward_ped": 3.50,
    ...
    def test_cancel_completed_quest_with_reward_undo_removes_ledger(self, quest_service: QuestService):
        q = quest_service.create_quest({
            "name": "Reward Quest",
            "cooldown_hours": 24,
            "reward_ped": 3.50,
    ...
    def test_cancel_completed_skill_quest_with_reward_undo_removes_claim(self, quest_service: QuestService):
...
class TestPlaylists:
    ...
    def test_create_playlist_with_long_horizon_group(self, quest_service: QuestService):
        ...
        assert [item["group_type"] for item in pl["items"]] == ["immediate", "immediate", "long_horizon"]
    ...
    def test_update_playlist_reorder(self, quest_service: QuestService):
        ...
        updated = quest_service.update_playlist(pl["id"], {
...
class TestMissionMatching:
    ...
    def test_exact_match(self, quest_service: QuestService):
        ...
        match = quest_service.match_quest_by_mission_name("Paneleon Hunter Jameson's Mission")
    ...
    def test_fuzzy_match_minor_typo(self, quest_service: QuestService):
        ...
        match = quest_service.match_quest_by_mission_name("Zyn'Nix Tempo - Duster Workeks")
        assert match is not None
    ...
    def test_fuzzy_no_match_too_different(self, quest_service: QuestService):
        ...
        quest_service.create_quest({"name": "Zyn'Nix Tempo - Duster Workers"})
        match = quest_service.match_quest_by_mission_name("Totally Unrelated Quest Name")
        assert match is None
    ...
    def test_fuzzy_picks_highest_score(self, quest_service: QuestService):
        ...
        match = quest_service.match_quest_by_mission_name("Zyn'Nix Tempo - Duster Workers")
        assert match is not None
        assert match["name"] == "Zyn'Nix Tempo - Duster Workers"
    ...
    def test_reward_filter_zero_ped_suppresses_lowest(self, quest_service: QuestService):
...
class TestAutoStartQuest:
    ...
    def test_event_bus_triggers_auto_start(self, tmp_path: Path):
        ...
        bus.publish(EVENT_MISSION_RECEIVED, {
...
class TestSessionQuestCompletions:
    ...
    def test_duplicate_completion_ignored(self, svc_with_events):
        ...
        svc.quest_reward_filter("Repeatable", [{"item_name": "UA", "quantity": 10000, "value": 1.0}], [])
        svc.quest_reward_filter("Repeatable", [{"item_name": "UA", "quantity": 10000, "value": 1.0}], [])
...
class TestPlaylistAnalytics:
    ...
    def test_economics_aggregation(self, svc_with_tables):
        svc, _ = svc_with_tables
        q1 = svc.create_quest({"name": "A", "reward_ped": 1.0, "expected_reward_markup_percent": 130.0})
        q2 = svc.create_quest({"name": "B", "reward_ped": 2.0})
        q3 = svc.create_quest({"name": "C", "reward_ped": 5.0, "expected_reward_markup_percent": 200.0})
        pl = svc.create_playlist({
            "name": "Loop",
            "items": [
                {"quest_id": q1["id"], "group_type": "immediate"},
...
class TestCuratedQuestAnalytics:
    ...
    def test_quest_analytics_ignore_raw_completions_until_curated(self, svc_with_tables):
        svc = svc_with_tables
        q = svc.create_quest({
            "name": "Solo",
            "reward_ped": 1.5,
    ...
    def test_playlist_analytics_ignore_raw_completions_until_curated(self, svc_with_tables):
        svc = svc_with_tables
        q1 = svc.create_quest({"name": "A", "reward_ped": 1.0})
        q2 = svc.create_quest({"name": "B", "reward_ped": 2.0})
        pl = svc.create_playlist({"name": "Loop", "quest_ids": [q1["id"], q2["id"]]})
