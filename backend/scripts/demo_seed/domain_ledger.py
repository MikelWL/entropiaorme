...
_EXPENSE_TAGS = ("equipment", "repair", "enhancers", "consumable", "armour", "transport", "other")
...
_INVENTORY_ITEMS: tuple[tuple[str, float, float, str, int], ...] = (
    ...
    ("Imperium Battle Vest, Sweat-Forged Plates",     0.00,   280.00, "Synth armour set, full body",       41),
)
...
_PRESETS: tuple[tuple[str, str, str, float, str], ...] = (
    ...
    ("Sold animal hide bundle",  "markup",  "Animal Hide x100, auction",  28.13, "item_sale"),
)
    ...
    def seed(self, refs: CanonicalRefs, db: sqlite3.Connection, data_dir: Path) -> None:
        ...
        for offset_days, description, amount in _INVENTORY_SALES:
            ...
            db.execute(
                "INSERT INTO ledger_entries (id, date, type, description, amount, tag) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (str(uuid.uuid4()), _iso_date(epoch), "markup", description, amount, "inventory_sale"),
            )
            entry_rows += 1
        ...
        log.info(
            ...
            entry_rows, inventory_rows, preset_rows,
        )
