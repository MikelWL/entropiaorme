...
//! `app/src-tauri/contracts/`, `app/src-tauri/fixtures/corpus/`, and
//! `app/src-tauri/eo-wire/tests/fixtures/`; the reports live at
//! `app/src-tauri/ratifications/<slug>.md`.
...
const RATIFICATIONS_PREFIX: &str = "app/src-tauri/ratifications/";
...
block) must be committed to app/src-tauri/ratifications/<slug>.md \
...
```\n";
    ...
    fn parses_real_committed_report_format() {
        let v = parse_ratification_artifact("app/src-tauri/ratifications/x.md", REAL_REPORT)
            .expect("verdict parsed");
        assert!(v.is_sound());
        assert_eq!(
            v.goldens,
            vec!["app/src-tauri/contracts/event_schemas.snapshot.json"]
        );
...
VERDICT: ratification-sound | regression-suspected | needs-user-judgement\n```";
    ...
    fn golden_paths_classified() {
        ...
        assert!(is_golden_path(
            "app/src-tauri/fixtures/corpus/scripted/basic_hunt_10_events/expected/fingerprint.jsonl"
        ));
        assert!(is_golden_path(
        ...
        assert!(!is_golden_path(
            "app/src-tauri/ratifications/typed-api-responses.md"
        ));
    }

    #[test]
    fn ratification_artifact_classified() {
        assert!(is_ratification_artifact("app/src-tauri/ratifications/x.md"));
        assert!(is_ratification_artifact(
            "app/src-tauri/ratifications/README.md"
        ));
        assert!(!is_ratification_artifact(
            "app/src-tauri/ratifications/notes.txt"
        ));
        assert!(!is_ratification_artifact(
    ...
    fn golden_set_keys() {
        assert_eq!(
            golden_set_key("app/src-tauri/contracts/event_schemas.snapshot.json"),
            "event_schemas"
        );
        assert_eq!(
            golden_set_key(
        ...
        assert_eq!(
            golden_set_key("app/src-tauri/eo-wire/tests/fixtures/normalizer_conformance.json"),
    ...
    fn verdict_covers_is_one_directional() {
        ...
        let oa = RatificationVerdict {
            path: "p.md".to_string(),
            verdict: "ratification-sound".to_string(),
            goldens: vec!["app/src-tauri/contracts/event_schemas.snapshot.json".to_string()],
            range: None,
        };
