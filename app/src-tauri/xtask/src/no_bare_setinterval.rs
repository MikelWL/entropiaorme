...
//!     `app/src/lib/realtime/useVisiblePoll.ts`. Every other timer-driven
...
//! (`.svelte`, `.ts`, and the `.js` family) under `app/src` (tracked-only
...
const SETINTERVAL_HOME: &str = "app/src/lib/realtime/useVisiblePoll.ts";
...
mod tests {
    ...
    fn flags_bare_setinterval_outside_home() {
        let f = scan_text("app/src/lib/foo.ts", "const id = setInterval(tick, 1000);");
        assert_eq!(f.len(), 1);
    ...
    fn tolerates_whitespace_and_qualified_form() {
        let f = scan_text("app/src/a.ts", "window.setInterval (fn, 100);");
        assert_eq!(f.len(), 1);
        assert_eq!(f[0].rule, "bare-setinterval");
    }
    ...
    fn does_not_flag_setinterval_substring() {
        ...
        let f = scan_text("app/src/a.ts", "mySetInterval(fn, 100);");
        assert!(f.is_empty());
    }

    #[test]
    fn flags_retired_event_anywhere() {
        let f = scan_text("app/src/a.ts", "listen('tracking-state-changed', handler);");
        assert_eq!(f.len(), 1);
    ...
    fn reports_correct_line_numbers() {
        let text = "line one\nsetInterval(fn, 1);\nline three";
        let f = scan_text("app/src/a.ts", text);
        assert_eq!(f.len(), 1);
