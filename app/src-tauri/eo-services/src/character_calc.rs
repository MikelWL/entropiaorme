...
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct OptimizerAttributeRow {
    pub name: String,
    pub weight: f64,
    pub current_level: f64,
...
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ExcludedSkillRow {
    pub name: String,
    pub weight: f64,
    pub reason: &'static str,
}

/// The path optimizer's breakdown (the facade adds the profession's
/// name).
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct PathOptimizerBreakdown {
    pub mode: &'static str,
    ...
    pub total_ped: f64,
    pub allocations: Vec<PathAllocationRow>,
    pub attributes: Vec<OptimizerAttributeRow>,
    pub excluded: Vec<ExcludedSkillRow>,
}

/// One regular-skill row of the HP optimizer.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct HpSkillRow {
    pub name: String,
    pub hp_increase: f64,
    pub current_level: f64,
...
/// The HP optimizer's breakdown.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct HpOptimizerBreakdown {
    pub current_hp: f64,
    pub skills: Vec<HpSkillRow>,
    pub attributes: Vec<HpAttributeRow>,
}
...
fn codex_fields(name: &str) -> (Option<&'static str>, Option<i64>) {
    match get_codex_category(name) {
        Some(category) => (Some(category), reward_divisor(category)),
        None => (None, None),
    }
}

/// Stable sort by a float key (ties keep entry order).
fn sort_stable_by_f64<T>(rows: &mut [T], key: impl Fn(&T) -> f64, descending: bool) {
    rows.sort_by(|a, b| {
        let ordering = key(a)
            .partial_cmp(&key(b))
...
pub fn profession_path_optimizer(
...
) -> Result<PathOptimizerBreakdown, PathOptimizerModeError> {
    ...
    let mut excluded: Vec<ExcludedSkillRow> = Vec::new();
    let mut attributes: Vec<OptimizerAttributeRow> = Vec::new();
    for (name, weight) in iter_profession_skills(profession) {
        if weight <= 0.0 {
            continue;
        ...
        if is_attribute(&name) {
            ...
            excluded.push(ExcludedSkillRow {
                name,
                weight,
                reason: "not unlocked",
            });
    ...
    for a in &attributes {
        end_prof += effective_points(&a.name, a.current_level) * a.weight;
    }
    end_prof /= 10000.0;
...
fn path_result(
    ...
    end_prof: f64,
    skills: &[PathSkill],
    attributes: Vec<OptimizerAttributeRow>,
    excluded: Vec<ExcludedSkillRow>,
) -> PathOptimizerBreakdown {
    let mut allocations: Vec<PathAllocationRow> = Vec::new();
    for s in skills {
        let (codex_category, codex_divisor) = codex_fields(&s.name);
        allocations.push(PathAllocationRow {
            name: s.name.clone(),
            weight: s.weight,
            current_level: s.current_level,
            levels_to_gain: round_half_even(s.allocated, 2),
            ped_cost: round_half_even(s.ped, 2),
            new_level: round_half_even(s.current_level + s.allocated, 2),
            codex_category,
            codex_divisor,
        });
    }

    // Filtered on the ROUNDED gain (the rows store rendered figures).
    let (mut allocated, mut unallocated): (Vec<PathAllocationRow>, Vec<PathAllocationRow>) =
        allocations
            .into_iter()
            .partition(|row| row.levels_to_gain > 0.0);
    sort_stable_by_f64(&mut allocated, |row| row.ped_cost, true);
    unallocated.sort_by(|a, b| a.name.cmp(&b.name));
    ...
    PathOptimizerBreakdown {
        ...
        current_level: round_half_even(current_prof, 2),
        end_level: round_half_even(end_prof, 2),
        profession_levels_gained: round_half_even(end_prof - current_prof, 2),
        total_ped,
...
pub fn hp_skill_optimizer(
    skill_levels: &Map<String, Value>,
    skills_data: &[Value],
) -> HpOptimizerBreakdown {
    ...
    let mut skills: Vec<HpSkillRow> = Vec::new();
    let mut attributes: Vec<HpAttributeRow> = Vec::new();
    ...
    for (name, hp_inc) in iter_hp_skills(skills_data) {
        ...
        if is_attribute(&name) {
            ...
            attributes.push(HpAttributeRow {
                name,
                hp_increase: hp_inc,
                current_level,
                levels_per_hp: round_half_even(levels_per_hp, 2),
            });
        } else {
            ...
            let (codex_category, codex_divisor) = codex_fields(&name);
            skills.push(HpSkillRow {
                name,
                hp_increase: hp_inc,
                current_level,
                levels_per_hp: round_half_even(levels_per_hp, 1),
                ped_per_hp: round_half_even(ped_per_hp, 2),
                hp_per_ped: round_half_even(hp_per_ped, 4),
                codex_category,
                codex_divisor,
            });
        }
    ...
    sort_stable_by_f64(&mut skills, |row| row.ped_per_hp, false);
    sort_stable_by_f64(&mut attributes, |row| row.levels_per_hp, false);

    HpOptimizerBreakdown {
        current_hp: round_half_even(current_hp, 2),
        skills,
        attributes,
    }
}
...
mod tests {
    ...
    /// The wire shape of a typed breakdown, for byte-shape assertions.
    fn to_json<T: Serialize>(value: T) -> Value {
        serde_json::to_value(value).expect("breakdown serialises")
    }
    use serde_json::json;
    ...
    fn path_optimizer_validates_inputs_and_partitions_results() {
        ...
        let result =
            to_json(profession_path_optimizer(&level_map, &profession(), Some(1.0), None).unwrap());
        ...
        let result =
            to_json(profession_path_optimizer(&level_map, &profession(), Some(0.0), None).unwrap());
        assert_eq!(result["totalPed"], 0.0);
        assert_eq!(result["professionLevelsGained"], 0.0);

        // Budget mode spends the whole budget.
        let result =
            to_json(profession_path_optimizer(&level_map, &profession(), None, Some(5.0)).unwrap());
    ...
    fn hp_calculations_gate_on_positive_levels_and_contributions() {
        ...
        let result = to_json(hp_skill_optimizer(&level_map, &skills_data));
    ...
    fn rich_fixture_outputs_match_the_backend_pins() {
        ...
        pin(
            &to_json(hp_skill_optimizer(&hp_levels, &skills_data)),
    ...
    fn greedy_ties_pick_the_first_skill_in_order() {
        ...
        let result =
            to_json(profession_path_optimizer(&level_map, &profession, None, Some(0.05)).unwrap());
        let allocations = result["allocations"].as_array().unwrap();
    ...
    fn zero_allocation_results_sort_alphabetically() {
        ...
        let result =
            to_json(profession_path_optimizer(&level_map, &profession, Some(0.1), None).unwrap());
    ...
    fn flat_curve_hp_costs_report_zero_efficiency() {
        ...
        let result = to_json(hp_skill_optimizer(&Map::new(), &skills_data));
    ...
    fn skills_at_the_curve_ceiling_cannot_allocate() {
        ...
        let result =
            to_json(profession_path_optimizer(&level_map, &profession, None, Some(50.0)).unwrap());
