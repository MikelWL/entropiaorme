...
mod tests {
    ...
    async fn baseline_creates_the_full_schema_surface_and_version_row() {
        ...
        // tables, and the map-pins table; index migrations: 18 baseline
        // + 4 analytical + the ledger date index + 2 market + 2 harvest
        // + the pin planet index = 28 indexes, 8 triggers.
        assert_eq!(count("table").await, 31);
        assert_eq!(count("index").await, 28);
    ...
    async fn schema_master_lists_the_real_objects_in_order() {
        ...
        // 31 declared tables (23 baseline + 3 daily-rollup projection +
        // 2 market feed + 2 harvest activity + map pins) + the migration
        // ledger + 28 indexes (18 baseline + 4 analytical + the ledger
        // date index + 2 market + 2 harvest + the pin planet index) +
        // 8 triggers (only SQLite's own bookkeeping is excluded; the
        // conformance comparison filters the ledger externally as its
        // one deliberate difference).
        assert_eq!(master.len(), 32 + 28 + 8);
