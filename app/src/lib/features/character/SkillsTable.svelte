			...
			<thead>
				<tr class="border-b border-border">
					<th class="py-2 px-3 text-left eyebrow cursor-pointer transition-colors duration-[var(--duration-fast)] hover:text-text" onclick={() => table.setSort('name')}>
						<span class="inline-flex items-center gap-1">Skill {#if table.sortKey === 'name'}<span class="text-accent">{table.sortDir === 'asc' ? '\u2191' : '\u2193'}</span>{/if}</span>
					</th>
					...
					<th class="py-2 px-3 text-right eyebrow cursor-pointer transition-colors duration-[var(--duration-fast)] hover:text-text" onclick={() => table.setSort('ttValue')}>
						<span class="inline-flex items-center gap-1">PES {#if table.sortKey === 'ttValue'}<span class="text-accent">{table.sortDir === 'asc' ? '\u2191' : '\u2193'}</span>{/if}</span>
					</th>
				</tr>
			</thead>
			<tbody>
