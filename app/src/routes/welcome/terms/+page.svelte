<script lang="ts">
	...
	async function complete() {
		...
		const newsSeen = await getPreference<boolean>('news_opt_in_seen', false);
		if (typeof sessionStorage !== 'undefined') {
			sessionStorage.setItem('welcome_just_finished', '1');
		}
		exiting = true;
		setTimeout(() => goto(newsSeen ? '/' : '/welcome/news-opt-in'), 380);
	}
...
<div class="shell" class:exiting>
	...
	<main class="stage">
		...
		>
			<div class="reprompt-banner">
				<span class="reprompt-banner-title">The Terms of Service and Privacy Policy have been updated.</span>
			</div>
			<TermsStep bind:accepted />
		</section>
	</main>
	...
	<footer class="bottom-bar">
		<div class="status-line" aria-hidden="true">
			...
			<span class="dot">·</span>
			<span>VERSION {CURRENT_TOS_VERSION}</span>
		</div>

		<div class="controls">
			<Button variant="primary" onclick={complete} disabled={!accepted}>
				<span>Continue</span>
