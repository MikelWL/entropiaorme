...
- **Rust dependencies** are pinned by the committed `Cargo.lock`. The continuous-integration policy job runs `cargo audit -D warnings` against the RustSec advisory database and `cargo deny check` against the policy in `frontend/src-tauri/deny.toml` (advisories, licences, and source allow-lists) on every change, so an advisory or a disallowed licence fails the build. A small number of transitive advisories are suppressed with a documented, per-advisory rationale in `frontend/src-tauri/.cargo/audit.toml`: each is either an unmaintained or unsound warning, or a vulnerability proven unreachable in the shipped Windows artefact and not yet fixable upstream. Where a reachability assumption underpins a suppression, it is pinned structurally (for example a `cargo deny` wrapper ban restricting a flagged crate to its sole legitimate parent), and a nightly job re-checks the suppressed vulnerability advisories after an in-range dependency update, failing the moment a fix becomes available so no suppression outlives its justification.
