...
- **Rust dependencies** are pinned by the committed `Cargo.lock`. The continuous-integration policy job runs `cargo audit -D warnings` against the RustSec advisory database and `cargo deny check` against the policy in `app/src-tauri/deny.toml` (advisories, licences, and source allow-lists) on every change, so an advisory or a disallowed licence fails the build. A small number of transitive advisories are suppressed with a documented, per-advisory rationale in `app/src-tauri/.cargo/audit.toml`: each is an unmaintained or unsound warning on a crate inside the upstream toolchain, not a vulnerability. Should a vulnerability-class suppression ever become necessary, the standing policy is that it must be proven unreachable in the shipped artefacts, pinned structurally (for example a `cargo deny` wrapper ban restricting the flagged crate to its sole legitimate parent), and re-checked by a scheduled job that fails the moment an upstream fix becomes available, so no suppression outlives its justification.
